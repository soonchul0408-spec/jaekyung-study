...
function persist() {
  localStorage.setItem('jaekyung-study-state', JSON.stringify({ currentId: state.currentId, favorites: state.favorites, studiedFrequencyKeys: state.studiedFrequencyKeys, examMonth: state.examMonth }))
}
...
function questionCard(question, compact = false) {
  return `<button class="question-card${compact ? ' compact' : ''}" data-open="${question.id}">
    ...
    ${question.reviewNeeded ? '<em>분류 검토 필요</em>' : ''}
  </button>`
}
...
function home() {
  ...
  return `<main class="screen home-screen">
    <p class="eyebrow">개인 학습용 · 2회분</p><h1>재경관리사<br/><i>기출 노트</i></h1>
    ...
    <nav class="home-menu">
      <button data-action="year"><span>01</span><strong>연도별 기출</strong><small>2025년 1월·3월 · 240문제</small><b>→</b></button>
      ...
      <button data-action="frequency"><span>04</span><strong>빈출 분석</strong><small>공개기출 8회분 · 핵심 TOP 12</small><b>→</b></button>
    </nav>
    <p class="source-note">출처: 삼일회계법인 2025년 1월·3월 재경관리사 기출문제 및 확정답안. 문항별 공식 해설은 제공되지 않았습니다.</p>
  </main>`
}
function year() {
...
function questionView() {
  const q = current(); const examQuestions = questions.filter((question) => question.examMonth === q.examMonth); const position = examQuestions.indexOf(q); const previous = examQuestions[position - 1]; const next = examQuestions[position + 1]
  return `<main class="screen question-screen"><header class="question-header">${button('‹', 'year', 'icon')}<span>${q.year}년 ${q.round} · ${subjectName(q.subjectId)}</span><button class="bookmark ${isFavorite(q.id) ? 'saved' : ''}" data-action="favorite">${isFavorite(q.id) ? '★' : '☆'}<small>헷갈림</small></button></header>
    <div class="question-meta"><span>${q.questionNo}번</span><div><b>${escapeHtml(topicName(q.primaryTopicId))}</b><i>${escapeHtml(q.concept)}${q.reviewNeeded ? ' · 분류 검토 필요' : ''}</i>${frequencyBadge(q)}</div></div>
    <article class="question-body"><div class="stem">${markEvidence(q.stem, q.evidence)}</div><ol class="options">${q.options.map((option, index) => { const marks = q.optionEvidence?.filter((item) => item.choiceNo === index + 1) || []; const marker = marks[0]; return `<li><span>${['①','②','③','④'][index]}</span><div>${markEvidence(option, state.evidence ? marks : [])}${state.evidence && marker ? `<small class="option-evidence ${marker.type}">${marker.type === 'correct' ? '정답 판단 표현' : '오답 확인 표현'}: “${escapeHtml(marker.text)}”</small>` : ''}</div></li>` }).join('')}</ol></article>
    <section class="learning-tools">
      ...
      ${q.explanationStatus === 'completed' ? button(state.explanation ? '정답 및 해설 접기' : '정답 및 해설 보기', 'explanation', state.explanation ? 'tool active-tool' : 'tool') : '<p class="explanation-pending">AI 학습용 해설은 순서대로 작성 중입니다.</p>'}
    </section>
    ...
    ${state.explanation && q.explanationStatus === 'completed' ? `<section class="note-panel caution"><p class="eyebrow">AI 학습용 해설</p><p class="answer-line">공식 확정답안: <strong>${['①','②','③','④'][q.answer - 1]} ${q.answer}번</strong></p><p>${escapeHtml(q.explanation)}</p><h3>판단 순서</h3><ol class="solution-steps">${q.solutionSteps.map((step) => `<li>${escapeHtml(step)}</li>`).join('')}</ol>${q.calculation ? `<h3>계산 과정</h3><p class="calculation-formula">${escapeHtml(q.calculation.formula)}</p><ol class="solution-steps">${q.calculation.substitutions.map((step) => `<li>${escapeHtml(step)}</li>`).join('')}</ol><p>결과: <strong>${escapeHtml(q.calculation.result)}</strong></p><h3>수험생용 풀이 설명</h3><p>${escapeHtml(q.calculation.teachingExplanation)}</p>` : ''}<h3>선택지 분석</h3><ul class="choice-analysis">${q.choiceAnalysis.map((item) => `<li><b>${['①','②','③','④'][item.choiceNo - 1]} ${item.verdict}</b><span>${escapeHtml(item.reason)}</span></li>`).join('')}</ul><h3>연결 개념</h3><p>${q.relatedConcepts.map(escapeHtml).join(' · ')}</p><h3>자주 하는 실수</h3><p>${escapeHtml(q.commonMistake)}</p><p class="explanation-disclaimer">공식 해설이 아닌, 제공된 지문과 확정답안을 바탕으로 작성한 AI 학습용 해설입니다.</p></section>` : ''}
...
function render({ preserveScroll = false } = {}) {
  ...
  app.innerHTML = state.page === 'home' ? home() : state.page === 'year' ? year() : state.page === 'review' ? review() : state.page === 'favorites' ? favorites() : state.page === 'frequency' ? frequency() : questionView()
